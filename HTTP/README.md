...
## 🧐 POST 기반 등록
- 회원 목록 조회 : ```/members```
  > GET, 정렬 필터, 조건은 쿼리 파라미터에
- 회원 단건 조회 : ```/members/{memberId}```
  > GET
- 회원 등록 : ```/members```
  > POST
...
- 파일 목록 조회 ```/files```
  > GET
- 파일 단건 조회 ```/files/{filename}```
  > GET
- 파일 등록 ```/files/{filename}```
  - POST와의 차이, URI를 클라이언트가 리소스의 위치를 지정한다.
  > PUT
...
- 회원 수정 폼 ```/members/{memberId}/edit```

### POST 사용
- 회원 등록 ```/members/new or /members```
- 회원 수정 ```/members/{memberId}/edit or /members/{memberId}```
...
3xx 응답의 결과에 Location 헤더가 있으면, Location 위치로 자동 리다이렉트 한다.
...
주문 완료 후 주문 내역 화면으로 이동. 리소스의 URI가 일시적으로 변경되는 것이므로, 검색 엔진 등에서 URL을 변경하면 안됨.     
 ...
 - #### 🖍 308 See Other
  ...
  1. POST로 주문 후에 결과 화면을 GET 메서드로 리다이렉트.    
...
클라이언트의 요청에 잘못된 문법등으로 서버가 요청을 수행할 수 없음. 오류의 원인이 클라이언트.
클라이언트가 잘못된 요청을 한 것이기 때문에 같은 요청을 반복해도 똑같이 실패한다. 요청을 수정하여 보내야 한다.
...
- 미래에 새로운 상태 코드가 추가되어도 클라이언트를 변하지 않아도 된다.
...
-  ### ️Accept-Encoding: 클라이언트가 선호하는 압축 인코딩.
...
- must-revalidate는 캐시 유효 간이라면 캐시를 사용한다.
  ...
  origin 서버에 접근할 수 없는 경우. no-cache 에서는 서버 설정에 따라 오래된 데이터를 결과를 받게될 수 있다.
