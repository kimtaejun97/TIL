...
POST 로 데이터를 전송하여 저장하거나 업데이트 한 경우 새로고침을 시도했을 때 동일 한 요청이 한번 더 전송되게 된다. 저장같은 경우에는
동일한 데이터를 아이디만 변경하여 한번 더 저장하게 되는 심각한 문제가 발생한다.    

이를 해결하기 위해 Redirect를 통해 POST 전송을 결과 페이지를 조회하는 GET 요청으로 변경해준다.
이 후에는 새로고침을 실행하여도 GET 요청을 통한 페이지 조회가 이루어진다.


## 🧐️ RedirectAttribute
리다이렉트를 할 때에는 Model에 전달할 데이터를 담을 수 없다. 이때 사용하는 것이 RedirectAttribute 이다.   
...
```
```addAttribute()``` 를 이용하면 ```{itemId}``` 와 같이 해당 값은 치환되어 URL에 들어가게 된다. 이때 값은 자동으로 인코딩 되어 들어간다. ID 값 같은 경우에는 상관 없지만 한글이 들어갈 경우
인코딩을 지정해주는 것은 중요하다. 보통 ```UrlEncoder.encode()``` 를 이용하여 인코딩 한다.
URL에 지정되지 않은 나머지 값(isSaved와 같은)은 쿼리 파라미터 형식으로 들어가게 된다.    
타임리프 에서는 쿼리 파라미터 값을 ```param.xxx``` 로 가져올 수 있다.

쿼리 파라미터 값으로 넘기지 않고 마치 Model과 같이 사용하고 싶다면 아래와 같이 사용할 수 있다.
```java
attributes.addFlashAttribute("isSaved", true);
```
flashAttribute()는 마치 모델과 같이 사용할 수 있으며, 일시적으로 사용하고 사라지게 된다.
