...
# 📌 네트워크 계층
***
> 네트워크 계층은 송신 단말에서 수신 단말로 패킷을 전송하는 역할을 담당.
> - 라우팅(제어 영역) : 패킷을 전송할 때 전체적인 패킷 전송 경로를 설정하는 것.
>   - 라우팅 알고리즘을 통해 포워딩 테이블을 결정.
>   - 다른 라우터와 소통하며 경로를 계산한다.
> - 포워딩(데이터 영역) : 입력 링크로 패킷이 들어왔을 때 적절한 출력 링크로 패킷을 이동시키는 것.
> 스위칭이라고도 불린다.
...
- 라우팅 알고리즘은 포워딩 테이블을 만드는 프로토콜을 말하며 Static한 방법과 software를 이용하는 방법이 있다.
1. Static
    - 네트워크 운영자가 직접 라우팅. 프로토콜을 사용하지 않기 때문에 software에 비해 비교적 오류가 많고,
  네트워크 변화에 대처하는 속도가 느리다.
     
2. SDN(Software Defined Networking)
    - 원격 컨트롤러가 포워딩 테이블을 계산하고 분배한다. 원격 컨트롤러는 라우터와 상호작용 하며 소프트에서 실행된다.
  

### ☝️ 라우터
- 라우터는 입력 포트, 출력 포트, 스위칭 구조, 라우팅 프로세스로 구성된다.
- 라우터는 입력 링크에 패킷이 도착하면 포워딩 테이블을 이용하여 출력 포트를 검색 한다.
포워딩 테이블은 라우팅 프로세스에 의해 구성되거나 SDN에서 전달 받는다.
- 포워딩 테이블에서 가장 긴 prefix와 일치하는 것을 찾은 후 해당 출력 포트로 보내기 위해 스위칭 구조로 패킷을 보낸다.
  
  #### ✏️ 스위칭
    - 입력 포트에서 출력 포트로 전달되는 과정이다.
    > - 메모리를 통한 스위칭 : 마치 I/O와 같이 동작하며 속도가 느려, 요즘은 사용하지 않는다.
    > - 단일 버스를 통한 스위칭 : 공유 버스를 통해 직접 출력 포트로 전달한다.
    > 모든 입력포트와 출력 포트가 하나를 공유하기 때문에 2개 이상의 패킷이 한번에 전송될 수 없다.
    > - 2N 버스를 통한 스위칭 : 2개의 버스를 사용하므로 2개의 입력 포트, 2개의 출력 포트로 서로다른 입,출력 포트에서 2개의 패킷을 한번에 전송할 수 있다.
    - 출력 포트에 패킷이 전달되면 포트 프로세싱을 통해 출력 링크로 전달된다.
  
  #### ✏️ 큐잉
  - 입력 포트에서 출력 포트로 패킷이 전달될 때 모든 패킷을 병렬로 전달할 수 없기 때문에 큐잉이 발생한다.
  - 입력 큐잉: 같은 출력 큐로 이동한다면 한번에 하나의 패킷만 전송할 수 있으므로, 뒤의 패킷은 이를 기다리게 된다.
  - 출력 큐잉: 스위칭이 매우 빠르게 동작할 경우 출력 포트에서 출력 링크로 패킷을 전송하는 속도보다 도착하는 속도가 더 빠를 수 있다.
  이 때 큐의 공간이 한정적이기 때문에 손실되는 패킷이 발생할 수 있고, 희생할 패킷을 선정해야 한다.
    > - FCFS: First Come First Served, 늦게온 패킷을 버린다.
    > - priority Queueing: 큐를 여러 클래스로 생성하고 우선순위에 따라 큐에 할당한고,
    > 우선순위가 높은 패킷부터 출력 링크로 전송한다.
    > - RR(Round Robin): 여러개의 큐 클래스를 생성하지만 우선순위를 부여하지 않고, 모든 클래스에서 번갈아가며 전송한다.
    > - WFQ(Weighted Fair Queueing): RR 방식에서 우선순위가 높은 큐 쪽에 높은 비율을, 낮은 큐에 낮은 비율을 할당하여 전송한다.
    
    
  #### ✏️ 서브넷 마스크로 주소 계산.
    - 서브넷 마스크가 128.119.40.128/26 일 떄.
    - prefix가 26이므로 앞의 26비트가 고정된다. 변경될 수 있는 비트는 6개 이므로, 가질 수 있는 IP주소 범위는
    128.119.40.128 ~ 128.119.40.191 이다.

# 📌 IP
***
### ☝️ IPv4
- 현재 주로 사용되는 IP 버전, 32 bit로 주소를 표기한다.(출발지, 목적지 IP주소)
- 4 bit로 데이터그램의 프로토콜 버전을 명시한다.
- 헤더는 20 byte ~ 60 byte의 가변 길이를 가지며 옵션을 포함하지 않는다면 보통 20byte의 헤더길이를 사용한다.
- 서비스 타입(실시간, 비 실시간 데이터그램), 식별자,flag, fragment offset 등 존재.
- 데이터그램의 길이: byte로 명시된 데이터그램의 길이.
- TTL(Time To Live): 8 bit의 크기를 가지며,라우터를 지날 때 마다 감소하여 0이되면 라우터에서 폐기한다.
  IP 데이터그램이 네트워크에서 무한 순회하는 것을 방지.
- Protocol: IP 데이터그램이 최종 목적지에 도착했을 때 상위 계층의 특정 프로토콜을 명시하기 위해 사용된다. (6은 TCP 17은 UDP 등..)
- 헤더 체크섬: 각 라우터마다 재 계산되어야 한다.
- 옵션 필드
- 데이터(payload)

### ☝️ Fragment
- 출력 링크의 크기가 데이터그램보다 작다면 IP 데이터그램의 페이로드를 더 작은 IP 데이터그램으로 나누어 전송해야 한다.
- Fragment 들의 재결합은 종단 시스템에서 구현되며 fragment offset, flag, 식별번호를 이용하여 같은 덩어리임을 구분한다.
- fragment offset: 하나의 데이터그램에서 어느 부분에 해당하는 단편인지.
- flag: 마지막 단편이면 0, 아직 남아있다면 1
- 식별 번호: 전송하는 데이터그램마다 식별번호를 증가시키며 전송하기 때문에 같은 덩어리임을 구분할 수 있게 해준다.
...
### ☝️ IPv6
- IPv4 가 거의 고갈 되자 등장한 새로운 버전의 IP 16진수로 표기하며 128 bit를 사용한다.
